...
from outguesser import Outguesser, simple_gradient_descent, maximum_a_posteriori, momentum_gradient_descent, regularized_momentum_gradient_descent,\
    linear_choice_history_dependent_model, choice_history_reward_history_model, linear_choice_history_dependent_model_predictor, \
    choice_history_reward_history_model_predictor
...
class Game:
    def add_trial(self, agent_choice, reward):
        ...
        self.outguesser.update_model(self.trials[[GameConstants.agent_choice, GameConstants.outguess_choice]].values.astype(int))

        self.number_of_trials += 1

    def get_outguesser_response(self):
        return self.outguesser.predict_next_choice(self.trials[[GameConstants.agent_choice, GameConstants.outguess_choice]].values.astype(int))
